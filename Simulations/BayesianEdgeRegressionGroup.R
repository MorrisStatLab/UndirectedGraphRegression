...
library('GIGrvg')
##load data
sid <- commandArgs(trailingOnly = T)
sid <- as.character(sid)[1]
load(paste0('sim',sid,'.rda'))
x = as.numeric(xx[,2])
xx = matrix(0, length(x), 2)
xx[x==0, 1] = 1
xx[x>0, 2] = 1
##############################################
x1 = xx[,1]; x2 = xx[,2]
x3 = rep(0, length(x)); x3[(x1 == 1)|(x2 == 1)] = 1
#account for joint graphc for three
xx = cbind(x1, x2, x3)
...
n1 = nrow(yy1);n2 = nrow(yy2);n3 = nrow(yy3)
